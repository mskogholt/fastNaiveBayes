...
fnb.multinomial.default <- function(x, y, priors = NULL, laplace = 0, sparse = FALSE, check = TRUE) {
  if(check){
    args <- fnb.multinomial.check.args.model(x, y, priors, laplace, sparse)
    x <- args$x
    y <- args$y
    priors <- args$priors
...
predict.fnb.multinomial <- function(object, newdata, type = c("class", "raw", "rawprob"), sparse = FALSE,
  ...
  type <- match.arg(type)
  if(check){
    args <- fnb.multinomial.check.args.predict(object, newdata, type, sparse, threshold, ...)
    object <- args$object
...
#' @import Matrix
fnb.multinomial.check.args.model <- function(x, y, priors, laplace, sparse){
  # x
...
#' @import Matrix
fnb.multinomial.check.args.predict <- function(object, newdata, type, sparse, threshold, silent = FALSE){
  if(threshold<0){
