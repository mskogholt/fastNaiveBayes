...
|-----------------------|-----------|----------|-------------|
| fastNaiveBayes        | 0.540     | 0.231       | 0.019    |
| fastNaiveBayes_sparse | 1.333     | 0.012       | 0.031    |
| bnlearn               | 5.976     |             |          |
| e1071                 | 522.618   |             | 1.405    |
| klar                  | 421.323   |             | 2.318    |
| naivebayes            | 4.247     |             | 0.021    |
| quanteda              | 8.075     | 0.174       |          |
| Rfast                 |           | 0.724       | 0.002    |
...
|-----------------------|-----------|-------------|----------|
| fastNaiveBayes        | 1.0       | 19.0        | 9.6      |
| fastNaiveBayes_sparse | 2.5       | 1.0         | 15.6     |
| bnlearn               | 11.1      |             |          |
| e1071                 | 967.1     |             | 707.2    |
| klaR                  | 779.6     |             | 1166.5   |
| naivebayes            | 7.9       |             | 10.7     |
...
As can be seen from the results, this package is magnitudes fast for both the Bernoulli and Multinomial event models. Using only a Bernoulli event model, the smallest speed-up was compared to the 'naivebayes' package, where this package was found to be 8 times faster. The largerst speed-up was compared to the 'klaR' and 'e1071' packages, where this package is around 780 and 967 times faster, respectively. 

For the Multinomial event model, there's only two alternative implementations, from the 'quanteda' and 'Rfast' package. This implementation was found to be 14 times and 60 times faster, respectively. 
...
It should be noted, that these results can between data sets and is dependent on both hardware and software. In order to make it easier to compare execution times, the tweets data that was used to establish these results are included in the package as 'tweets', the raw data, and 'tweetsDTM' a clean document-term matrix of the previously mentioned 'tweets' data. The code used to convert the raw 'tweets' data can be found on github in the 'data-raw' folder. Moreover, the code to establish the results can be found below.
