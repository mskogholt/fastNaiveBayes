...
# Introduction
Text classification is the task of classifying documents by their content: that is, by the words of which they are comprised. The documents are often represented as a bag of words. This means that only the occurrence and frequency of the words in the document are taken into account, any information about the syntactic structure of these words is discarded \ref{4}. In many research efforts regarding document classification, Naive Bayes has been successfully applied \ref{5}. 

This package is a very fast implementation of the Naive Bayes classifier with the option to use two feature modelling distributions, as will be explained later on. 

# Naive Bayes 
Naive Bayes is a probabilistic classification method based on the Bayes theorem with a strong and naive independence assumption. Naive Bayes assumes independence between all attributes. Despite this so-called “Naive Bayes assumption”, this technique has been proven to be very effective for text classification \ref{6}. In the context of text classification, Naive Bayes estimates the posterior probability that a document, consisting out of several words, belongs to a certain class and classifies the document as the class which has the highest posterior probability:
